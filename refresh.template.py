...
def _parse_headers_from_makefile_deps(
    ...
    assert source_path_for_sanity_check is None or source.endswith(
        source_path_for_sanity_check
    ), "Something went wrong in makefile parsing to get headers. The first dependency should be the source file. Output:\n" + d_file_content
...
def _get_headers_gcc(compile_action, source_path: str, action_key: str):
    ...
    ):  # Safe because Bazel only holds one cached action key per path, and the key contains the path.
        for i, arg in enumerate(compile_action.arguments):
            if arg.startswith('-MF'):
                ...
                if os.path.isfile(dep_file_path):
                    dep_file_last_modified = os.path.getmtime(
                        dep_file_path
                    )  # Do before opening just as a basic hedge against concurrent write, even though we won't handle the concurrent delete case perfectly.
    ...
    if is_nvcc:
        ...
        header_cmd += [
            '-M', '--print-missing-file-dependencies'
        ]  # Allows us to continue on past missing (generated) files--whose paths may be wrong (listed as written in the include)!
    ...
    if is_nvcc:
        should_cache = bool(
            header_search_process.stdout
        )  # Without --print-missing-file-dependencies, nothing is written if a file isn't found. (Something is still written if no headers are included.) This avoids hardcoding the error message. Note that some errors, like that for #bad_directive are okay to ignore! We'll know the cache isn't fresh when the user changes the file.
        ...
        }  # We can't filter on headers starting with bazel-out because missing headers don't have complete paths; they just listed as #included
...
def _subprocess_run_spilling_over_to_param_file_if_needed(
    ...
    try:
        ...
        if e.winerror == 206:  # Thrown when command is too long, despite the error message being "The filename or extension is too long". For a few more details see also https://stackoverflow.com/questions/2381241/what-is-the-subprocess-popen-max-length-of-the-args-parameter
            ...
            try:
                os.write(fd,
                         windows_list2cmdline(command[1:]).encode()
...
def _get_headers_msvc(compile_action, source_path: str):
    ...
    for line in header_search_process.stderr.splitlines():
        ...
        if source_path.endswith(
                '/' + line
        ) or source_path == line:  # Munching the source filename echoed the first part of the include output
            continue
    ...
    should_cache = all(
        'fatal error C1083:' not in error_line for error_line in error_lines
    )  # Error code for file not found (when trying to include). Without this, we'd wrongly get a subset of the headers and we might wrongly think the cache is still fresh because we wouldn't know that the formerly missing header had been generated.
    return headers, should_cache
...
def _get_headers(compile_action, source_path: str):
    ...
    if {exclude_headers} == "all":
        return set()
    elif {exclude_headers
          } == "external" and not {exclude_external_sources
    ...
    for i, arg in enumerate(compile_action.arguments):
        ...
        if arg == '-o' or arg == '--output':  # clang/gcc. Docs https://clang.llvm.org/docs/ClangCommandLineReference.html
            ...
            break
        elif arg.startswith('/Fo') or arg.startswith(
                '-Fo'
        ):  # MSVC *and clang*. MSVC docs https://docs.microsoft.com/en-us/cpp/build/reference/compiler-options-listed-alphabetically
            output_file = arg[3:]
            break
    ...
    if not output_file and not _get_headers.has_logged:
        ...
        log_warning(
            f""">>> Please file an issue containing the following: Output file not detected in arguments {compile_action.arguments}.
    ...
    if output_file:
        ...
        if os.path.isfile(cache_file_path):
            cache_last_modified = os.path.getmtime(
                cache_file_path
            )  # Do before opening just as a basic hedge against concurrent write, even though we won't handle the concurrent delete case perfectly.
            try:
...
def _get_files(compile_action):
    ...
    if not os.path.isfile(source_file):
        if not _get_files.has_logged_missing_file_error:  # Just log once; subsequent messages wouldn't add anything.
            ...
            log_warning(
                f""">>> A source file you compile doesn't (yet) exist: {source_file}
    ...
    if os.path.splitext(
            source_file)[1] in _get_files.assembly_source_extensions:
        return {source_file}, set()
...
def _get_apple_SDKROOT(SDK_name: str):
    ...
    version = subprocess.check_output(
        ...
        stderr=subprocess.DEVNULL,
        encoding=locale.getpreferredencoding()).rstrip()
    return SDKROOT_maybe_versioned.replace(
        version, ''
    )  # Strip version and use unversioned SDK symlink so the compile commands are still valid after an SDK update.
...
def _apple_platform_patch(compile_args: typing.List[str]):
    ...
    if any('__BAZEL_XCODE_' in arg for arg in compile_args):
        ...
        ]  # No need for debug prefix maps if compiling in place, not that we're compiling anyway.
...
def _nvcc_patch(compile_args: typing.List[str]) -> typing.List[str]:
    ...
    for arg in compile_args:
        ...
        for flag_with_arg in _nvcc_flags_to_skip_with_arg:
            ...
            if len(
                    flag_with_arg
            ) > 2:  # NVCC allows all one-character flags to have no = or space separator before their arguments
                flag_with_arg += '='
        ...
        rewrite_to = _nvcc_rewrite_flags.get(
            option
        )  # Handles =value. Note: Note assumes no rewritten args are one character with a value that might take the -I<value> short form. <TAG_REWRITE>
        if rewrite_to:
            if did_partition:
...
_nvcc_flags_to_skip_no_arg = {
    ...
    '-forward-unknown-to-host-linker',  # Not handled quite right--but very hard to handle fully. If patching, consider adding a special case prepending '-Wl,', making sure to not trigger the list expander.
    '--gen-opt-lto',
    '-gen-opt-lto',
    '--generate-line-info',
    '-lineinfo',
    ...
    if {exclude_headers} == "external" and not {exclude_external_sources}:
        ...
        for action in aquery_output.actions:
            ...
            target = targets_by_id[
                action.
                targetId]  # Should always be present. KeyError as implicit assert.
            assert not target.startswith(
                '//external'
            ), f"Expecting external targets will start with @. Found //external for action {action}, target {target}"
...
def _get_commands(target: str, flags: str):
    ...
    missing_targets_warning: typing.Pattern[str] = re.compile(
        r'(\(\d+:\d+:\d+\) )?(\033\[[\d;]+m)?WARNING: (\033\[[\d;]+m)?Targets were missing from graph:'
    )  # Regex handles --show_timestamps and --color=yes. Could use "in" if we ever need more flexibility.
    ...
    try:
        ...
        log_warning(
            f">>> Failed extracting commands for {target}\n    Continuing gracefully..."
        )
        return

    if not getattr(
            parsed_aquery_output, 'actions',
            None):  # Unifies cases: No actions (or actions list is empty)
        if aquery_process.stderr:
            log_warning(
                f""">>> Bazel lists no applicable compile commands for {target}, probably because of errors in your BUILD files, printed above.
    Continuing gracefully...""")
        else:
            log_warning(
...
def _ensure_external_workspaces_link_exists():
    ...
    if os.path.lexists(
        ...
        try:
            ...
            log_error(
                f">>> //external already exists, but it isn't a {'junction' if is_windows else 'symlink'}. //external is reserved by Bazel and needed for this tool. Please rename or delete your existing //external and rerun. More details in the README if you want them."
...
def _ensure_gitignore_entries_exist():
    ...
    git_dir_process = subprocess.run(
        'git rev-parse --git-common-dir',  # common-dir because despite current gitignore docs, there's just one info/exclude in the common git dir, not one in each of the worktree's git dirs.
        shell=
        True,  # Ensure this will still fail with a nonzero error code even if `git` isn't installed, unifying error cases.
        stdout=subprocess.PIPE,
        stderr=subprocess.DEVNULL,
        encoding=locale.getpreferredencoding(),
    )
    ...
    with open(hidden_gitignore_path, 'a+') as gitignore:
        gitignore.seek(
            0
        )  # Files opened in `a` mode seek to the end, so we reset to the beginning so we can read.
...
def _ensure_cwd_is_workspace_root():
    ...
    try:
        ...
        log_error(
            ">>> BUILD_WORKSPACE_DIRECTORY was not found in the environment. Make sure to invoke this tool with `bazel run`."
        )
        sys.exit(1)
